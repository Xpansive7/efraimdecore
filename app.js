...
function handleDocumentClick(event) {
  ...
  if (action === "export-budget-pdf") {
    ...
    const type   = event.target.dataset.pdfType || "interno";
    if (budget) exportBudgetPDF(budget, type);
    return;
  }
...
function exportBudgetPDF(budget, type) {
  const client   = getClientById(budget.clientId);
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .pill{display:inline-block;padding:3px 10px;border-radius:999px;font-size:11px;font-weight:600;background:#f0f0f0;color:#555;border:1px solid #ddd}
  ...
  td{padding:8px;border-bottom:1px solid #f0f0f0;font-size:12px;color:#333}
  .section-title{font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:#999;font-weight:600;margin:20px 0 8px}
  ...
  if (type === "interno") {
    ...
    const barW = Math.min((margin / 60) * 100, 100).toFixed(1);
    const matRows = budget.materials.length
      ...
      : `<tr><td colspan="4" style="color:#aaa;font-style:italic">Nenhum material listado.</td></tr>`;

    body = `
    ${header}
    <div style="display:inline-block;padding:3px 10px;border-radius:999px;font-size:11px;font-weight:600;background:${tierColor}22;color:${tierColor};border:1px solid ${tierColor}55;margin-bottom:12px">
    ...
    <div class="grid grid-3">
      ...
      <div class="card"><span>Lucro previsto</span><strong style="color:${tierColor}">${formatCurrency(profit)}</strong></div>
      <div class="card"><span>Margem</span><strong style="color:${tierColor}">${formatPercent(margin)} · ${tierLabel}</strong></div>
      <div class="card"><span>Prazo</span><strong>${budget.businessDays} dias úteis</strong></div>
      <div class="card"><span>Status</span><strong>${escapeHtml(budget.status)}</strong></div>
    </div>
    <div style="height:8px;border-radius:4px;background:#e8e8e8;margin:4px 0 18px;overflow:hidden">
      <div style="height:100%;border-radius:4px;background:${tierColor};width:${barW}%"></div>
    </div>
    ${budget.description ? `<div class="section-title">Observações internas</div><div class="notes">${escapeHtml(budget.description)}</div>` : ""}
    ...
    <div class="footer">
      <span>Efraim Decore · Documento Interno</span>
      <span>Não compartilhar com o cliente</span>
    </div>`;
  } else {
    const matRows = budget.materials.length
      ? budget.materials.map((m) => `<tr><td>${escapeHtml(m.name||"")}</td><td>${escapeHtml(m.quantity||"—")}</td></tr>`).join("")
      : `<tr><td colspan="2" style="color:#aaa;font-style:italic">A definir com o cliente.</td></tr>`;

    body = `
    ${header}
    ${clientInfo}
    <div class="grid grid-2">
      <div class="card"><span>Valor total</span><strong>${formatCurrency(budget.price)}</strong></div>
      <div class="card"><span>Prazo de execução</span><strong>${budget.businessDays} dias úteis</strong></div>
    </div>
    ${budget.description ? `<div class="section-title">Escopo do projeto</div><div class="notes">${escapeHtml(budget.description)}</div>` : ""}
    <div class="section-title">Materiais inclusos</div>
    <table>
      <thead><tr><th>Item</th><th>Quantidade</th></tr></thead>
      <tbody>${matRows}</tbody>
    </table>
    <div style="margin-top:24px;padding:14px 16px;border:1px solid #e8e8e8;border-radius:8px;background:#fafafa;font-size:12px;color:#555;line-height:1.7">
      Este orçamento é válido por <strong>15 dias</strong> a partir da data de emissão.<br/>
      Para aprovação, entre em contato pelo número informado.
    </div>
    <div class="footer">
...
<title>${type === "interno" ? "[INTERNO] " : ""}Orçamento – ${escapeHtml(budget.title)}</title>
<style>${baseStyles}</style></head><body>${body}
